...
int main()
{
    ...
    for(int it = 0; t<tLimit; ++it) {   
        ...
        for(int i=1; i<N; ++i){
            if(c*uOld[i] >= 0){
                uNew[i] = uOld[i] - c * uOld[i] * dt / dx * (uOld[i] - uOld[i-1]) + D * dt / (dx * dx) * (uOld[i-1] - 2.0 * uOld[i] + uOld[i+1]);
            }else{
                uNew[i] = uOld[i] - c * uOld[i] * dt / dx * (uOld[i+1] - uOld[i]) + D * dt / (dx * dx) * (uOld[i-1] - 2.0 * uOld[i] + uOld[i+1]);
            }    
        }

        if(c * uOld[0] >= 0){
            uNew[0] = uOld[0] - c * uOld[0] * dt / dx * (uOld[0] - uOld[N]) + D * dt / (dx * dx) * (uOld[1] - 2.0 * uOld[0] + uOld[N]);
        }else if(c * uOld[0] < 0){
            uNew[0] = uOld[0] - c * uOld[0] * dt / dx * (uOld[1] - uOld[0]) + D * dt / (dx * dx) * (uOld[1] - 2.0 * uOld[0] + uOld[N]);
        }
            
        if(c * uOld[N] >= 0){
            uNew[N] = uOld[N] - c * uOld[N] * dt / dx * (uOld[N] - uOld[N-1]) + D * dt / (dx * dx) * (uOld[0] - 2.0 * uOld[N] + uOld[N-1]);
        }else if(c * uOld[0] < 0){
            uNew[N] = uOld[N] - c * uOld[N] * dt / dx * (uOld[N+1] - uOld[N]) + D * dt / (dx * dx) * (uOld[0] - 2.0 * uOld[N] + uOld[N-1]);
        }
